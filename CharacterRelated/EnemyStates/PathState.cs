...
public class PathState : IState
{
    ...
    public void Enter(Enemy parent)
    {
        ...
        targetPos = Player.m_instance.MyCurrentTile.position;
    ...
    public void Update()
    {
        if (parent.MyPath != null)
        {
            ...
            else if (Player.m_instance.MyCurrentTile.position == parent.MyCurrentTile.position)
            {
                parent.ChangeState(new FollowState());
            }
            else if (!parent.InRange)
            {
                parent.ChangeState(new EvadeState());
            }
            ...
            if (distance <= 0.0f)
            {
                if (parent.MyPath.Count > 0)
                {
                    ...
                    if (targetPos != Player.m_instance.MyCurrentTile.position)
                    {
